...
const LarpDvärgh: Room<State> = function ({ player }) {
  ...
  const historyOnChoose: () => Consequence = () => {
    player.attributes.push("stalinist")
    return {
      text:
          ...
          room: () => ({
            ...
            choices: [
              ...
              },
              {
                ...
                onChoose: (["matematiker", "fysiker"] as Character[]).includes(player.character)
                  ? historyOnChoose
                  : () => ({ room: LarpDvärghBattle }),
              },
            ],
